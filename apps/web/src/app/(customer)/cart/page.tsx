    ...
    return (
      <main className="container flex flex-col items-center gap-4 px-4 py-20 text-center sm:px-6">
        <span className="flex size-16 items-center justify-center rounded-full bg-(--color-muted)">
          <ShoppingCart className="size-7 text-(--color-muted-foreground)" aria-hidden />
        </span>
              ...
              >
                ...
                >
                  {item.product.imageUrl ? (
                    ...
                    <div className="flex size-full items-center justify-center">
                      <Package className="size-6 text-(--color-muted-foreground)" aria-hidden />
                    </div>
                  )}
                </Link>
                ...
                <div className="flex flex-1 flex-col gap-1">
                  ...
                  <div className="mt-auto flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 rounded-(--radius-inner) border border-(--color-primary) px-1.5 py-1">
                      <button
                        type="button"
                      ...
                      <span className="min-w-[2ch] text-center text-sm font-semibold text-(--color-primary)">
                        {item.quantity}
                      </span>
                      <button
                        type="button"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        disabled={isItemMutating}
                        onClick={() => removeItem.mutate(item.id, { onError: onCartError })}
        ...
        <div className="flex h-fit flex-col gap-3 rounded-(--radius-outer) border border-(--color-border) bg-(--color-card) p-4">
          ...
          <Button
            ...
            disabled={cart.items.some(
              (item) => !item.availability.inStock || item.availability.availableQuantity < item.quantity,
            )}
